...
public class DataTests
{
    ...
    public async Task UserManagementTest()
    {
        ...
        try
        {
            var contextOptions = new DbContextOptionsBuilder<BlogsContext>()
                .LogTo(Console.WriteLine).EnableSensitiveDataLogging().EnableDetailedErrors()
                .UseSqlite(connection)
                .Options;

            await using (var context = new BlogsContext(contextOptions))
            {
    ...
    public async Task ArticleManagementTest()
    {
        ...
        try
        {
            ...
            {
                var repo = new BlogRepository(context);
                var articles = await repo.GetArticlesAsync(new ArticlesQuery(null),
                    username1,
                    false,
